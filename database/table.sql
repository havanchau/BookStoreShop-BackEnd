...
CREATE TABLE customers ( id VARCHAR(20) PRIMARY KEY NOT NULL, customerName VARCHAR(50) NOT NULL, physicalAddress VARCHAR(50), totalPayment FLOAT, userId VARCHAR(20) NOT NULL, FOREIGN KEY (userId) REFERENCES users(id)
ON UPDATE CASCADE
ON
DELETE CASCADE );

CREATE TABLE orders ( id VARCHAR(20) PRIMARY KEY NOT NULL, numberProduct INT NOT NULL, cost FLOAT NOT NULL, time_delivery DATE NOT NULL, customerId VARCHAR(20) NOT NULL, employeeId VARCHAR(20) NOT NULL, deliveryAddress VARCHAR(50), deliveryFee FLOAT FOREIGN KEY (customerId) REFERENCES customers(id)
ON UPDATE CASCADE
ON
DELETE CASCADE, FOREIGN KEY (employeeId) REFERENCES employees(id)
ON UPDATE CASCADE
ON
DELETE CASCADE, );

CREATE TABLE order_contain_book ( idOrder VARCHAR(20), isbn VARCHAR(20), numberBook INT, PRIMARY KEY (idOrder, isbn), FOREIGN KEY (idOrder) REFERENCES orders(id)
ON UPDATE CASCADE
ON
...
CREATE TABLE transportTos ( idWarehouse VARCHAR(20) NOT NULL, idPackage VARCHAR(20) NOT NULL, arrivalTime DATE NOT NULL, PRIMARY KEY (ID_Ware_House, ID_Package), FOREIGN KEY (idWarehouse) REFERENCES warehouses(id)
ON UPDATE CASCADE
ON
DELETE CASCADE, FOREIGN KEY (idPackage) REFERENCES packages(id)
ON UPDATE CASCADE
...
CREATE TABLE transportFroms ( idWarehouse VARCHAR(20) NOT NULL, idPackage VARCHAR(20) NOT NULL, departureTime DATE NOT NULL, PRIMARY KEY (ID_Ware_House, ID_Package), FOREIGN KEY (idWarehouse) REFERENCES warehouses(id)
ON UPDATE CASCADE
ON
DELETE CASCADE, FOREIGN KEY (idPackage) REFERENCES packages(id)
ON UPDATE CASCADE
...
ON UPDATE CASCADE
ON
DELETE CASCADE, FOREIGN KEY (idCustomer) REFERENCES customers(idCustomer)
ON UPDATE CASCADE
ON
